...
2. **Product behavior** — what **MigrationsKitBundle** guarantees to applications that integrate it (see [`USAGE.md`](USAGE.md), [`CONFIGURATION.md`](CONFIGURATION.md), [`INSTALLATION.md`](INSTALLATION.md)). **PHPUnit** and **PHPStan** enforce contracts in CI where applicable.
...
| ID | Story |
| --- | --- |
| US-01 | **As a** migration author, **I want** `SchemaChecker` **so that** I guard migrations with `tableExists` / `columnExists` checks. |
| US-02 | **As a** migration author, **I want** MDK declarative arrays **so that** I avoid hand-written DDL for common schema changes. |
| US-03 | **As a** migration author, **I want** ordered drop/add phases **so that** FK/index/column operations stay idempotent. |
| US-04 | **As a** maintainer, **I want** demos on Symfony 7 and 8 **so that** DBAL 3/4 compatibility is proven in CI. |
...
**Goal:** Symfony bundle providing migration helpers: schema checks (table/column/index exists) and array-based migration definitions. Use in Doctrine Migrations with Symfony 7|8.
