...
4. **Transactional mode and DDL on MySQL**: By default Doctrine Migrations run each migration in a transaction. On MySQL, DDL (CREATE TABLE, ALTER TABLE, DROP, etc.) causes implicit commits, which triggers a **deprecation warning** (“the transaction is already committed, relying on silencing is deprecated”). To avoid it and align with Doctrine’s recommendation, use `transactional: false` in config or override `isTransactional()` to return `false` in migrations that touch the schema. See [USAGE.md — Transaction already committed (deprecation)](USAGE.md#transaction-already-committed-deprecation-when-using-ddl-on-mysql) and [Doctrine’s implicit commits](https://www.doctrine-project.org/projects/doctrine-migrations/en/stable/explanation/implicit-commits.html).
