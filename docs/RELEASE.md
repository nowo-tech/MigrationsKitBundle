...
## Creating a new version (e.g. v1.2.1)

1. **Ensure everything is ready**
   - [CHANGELOG.md](CHANGELOG.md) has the target version (e.g. `[1.2.1]`) with date and full entry; `[Unreleased]` is at the top and empty or updated for the next cycle.
...
2. **Commit and push** any last changes to your default branch (e.g. `main` or `master`):
   ```bash
   git add -A
   git commit -m "Prepare v1.2.1 release"
