...
Documentation and changelog are prepared for **v2.0.17**. Before tagging, ensure the lock file is valid:
...
3. **Commit, push, and tag**:
   ```bash
   git add -A
   git commit -m "Prepare v2.0.17 release"
...
4. **(Optional)** Open GitHub → Releases → Draft a new release from tag `v2.0.17` and paste the [2.0.17] section from [CHANGELOG.md](CHANGELOG.md).
...
Documentation and changelog are prepared for **v2.0.14**. Before tagging, ensure the lock file is valid:
...
3. **Commit, push, and tag**:
   ```bash
   git add -A
   git commit -m "Prepare v2.0.14 release"
   git push origin master
   git tag -a v2.0.14 -m "Release v2.0.14"
   git push origin v2.0.14
   ```

4. **(Optional)** Open GitHub → Releases → Draft a new release from tag `v2.0.14` and paste the [2.0.14] section from [CHANGELOG.md](CHANGELOG.md).
...
Documentation and changelog are prepared for **v2.0.12**. Before tagging, ensure the lock file is valid:
...
3. **Commit, push, and tag**:
   ```bash
   git add -A
   git commit -m "Prepare v2.0.12 release"
   git push origin master
   git tag -a v2.0.12 -m "Release v2.0.12"
   git push origin v2.0.12
   ```

4. **(Optional)** Open GitHub → Releases → Draft a new release from tag `v2.0.12` and paste the [2.0.12] section from [CHANGELOG.md](CHANGELOG.md).

---

## Release v2.0.11 (ready)

Documentation and changelog are prepared for **v2.0.11**. Before tagging, ensure the lock file is valid:
...
3. **Commit, push, and tag**:
   ```bash
   git add -A
   git commit -m "Prepare v2.0.11 release"
   git push origin master
   git tag -a v2.0.11 -m "Release v2.0.11"
   git push origin v2.0.11
   ```

4. **(Optional)** Open GitHub → Releases → Draft a new release from tag `v2.0.11` and paste the [2.0.11] section from [CHANGELOG.md](CHANGELOG.md).

---

## Release v2.0.10 (ready)

Documentation and changelog are prepared for **v2.0.10**. Before tagging, ensure the lock file is valid:
...
3. **Commit, push, and tag**:
   ```bash
   git add -A
   git commit -m "Prepare v2.0.10 release"
   git push origin master
   git tag -a v2.0.10 -m "Release v2.0.10"
   git push origin v2.0.10
   ```

4. **(Optional)** Open GitHub → Releases → Draft a new release from tag `v2.0.10` and paste the [2.0.10] section from [CHANGELOG.md](CHANGELOG.md).
...
- **Scope:** Fix DBAL 3 CI fatal from typed FK stubs in `SchemaDefinitionParserTest` (introduced in 2.0.16); exclude that file from Rector. No runtime or API changes.
- **Checklist:** CHANGELOG and UPGRADING updated. Run `make composer-sync`, then `make release-check`, commit, `make check-no-cursor-coauthor`, push, and tag v2.0.17.
...
- **Checklist:** CHANGELOG and UPGRADING updated. Run `make composer-sync`, then `make release-check`, commit, push, and tag v2.0.14.
...
- **Scope:** Fix DBAL 3 CI (`test-dbal`, `coverage-dbal3`) — PHPUnit `Schema`/`Table` stub signatures compatible with DBAL 3 untyped parameters. No runtime or API changes.
- **Checklist:** CHANGELOG and UPGRADING updated. Run `make composer-sync`, then `make release-check`, commit, push, and tag v2.0.12.

---

## v2.0.11 (2026-06-17)

- **Scope:** PHPStan level 8 clean; internal refactors in **TableSchemaHelper** and **SchemaDefinitionParser**; expanded PHPUnit coverage (~99% lines, **CreateTablesService** at 100%). No API or migration SQL changes.
- **Checklist:** CHANGELOG and UPGRADING updated. Run `make composer-sync`, then `make release-check`, commit, push, and tag v2.0.11.

---

## v2.0.10 (2026-06-17)

- **Scope:** CI matrix refined (PHP 8.2–8.5 × Symfony 7|8; `test-dbal` for DBAL 3|4); PHPUnit stubs and MySQL drop-column assertion compatible with DBAL 3.
- **Checklist:** CHANGELOG and UPGRADING updated. Run `make composer-sync`, then `make release-check`, commit, push, and tag v2.0.10.
